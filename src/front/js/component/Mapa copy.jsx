...
export const Mapa = ({ direccion }) => {
  ...
  // Cargar el script de Google Maps y la Geocoding API
  ...
  // Verifica si Google Maps se ha cargado
  if (!isLoaded) {
    return <div>Cargando...</div>;
  }

  // Si las coordenadas no se han obtenido, mostramos un mapa centrado por defecto
  const defaultCenter = { lat: 19.4326, lng: -99.1332 }; // México por defecto
  ...
  return (
    <GoogleMap
      ...
      zoom={14}
      center={coordenadas || defaultCenter} // Centro del mapa con coordenadas obtenidas o por defecto
    >
